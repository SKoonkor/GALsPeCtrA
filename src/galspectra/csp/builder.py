...
def build_csp_coefficients(
        ...
        target_Z,
        mass_weights):
    """Build CSP coefficients in PCA space.

    interpolator : PCACoefficientInterpolator
    target_ages : (N_bins,) linear ages per mass bin
    target_Z : (N_bins,) or scalar
    mass_weights : (N_bins,) stellar mass formed in each bin

    Returns csp_coeffs : (N_pc,)
    """
